...
# def parse_reference_number(reference_number):
#     try:
#         return int(float(reference_number))
#     except ValueError:
#         return None

# def is_valid_reference_number(reference_number):
#     try:
#         num = int(reference_number)
#         return 1000000 <= num <= 9999999
#     except ValueError:
#         return False
...
#         logging.warning("Reference number is missing in the request")
...
#     # Convert reference number to int for comparison
#     try:
#         reference_number_int = int(reference_number)
#     except ValueError:
#         logging.warning(f"Invalid reference number format: {reference_number}")
#         abort(400, description="Invalid reference number format. Must be an integer.")

#     # Ensure reference_number is a string for comparison
#     load = next((load for load in loads if int(load['reference_number']) == reference_number_int), None)
...
def get_load_by_reference():
    ...
    try:
        reference_number_int = int(float(reference_number))  # Handle scientific notation
        if not 1000000 <= reference_number_int <= 9999999:
            logging.warning(f"Invalid reference number format: {reference_number}")
            abort(400, description="Invalid reference number. Must be a 7-digit number.")
    except ValueError:
        logging.warning(f"Invalid reference number format: {reference_number}")
        abort(400, description="Invalid reference number format. Must be an integer.")
    ...
    # Ensure reference_number is an integer for comparison
    load = next((load for load in loads if int(load['reference_number']) == reference_number_int), None)
